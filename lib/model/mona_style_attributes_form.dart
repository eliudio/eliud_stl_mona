  ...
  Widget build(BuildContext context) {
    ...
    if (formAction == FormAction.showData) {
      return BlocProvider<MonaStyleAttributesFormBloc >(
        ...
        child: _MyMonaStyleAttributesForm(app:app, submitAction: submitAction, formAction: formAction),
          );
...
class _MyMonaStyleAttributesForm extends StatefulWidget {
  ...
  _MyMonaStyleAttributesForm({required this.app, this.formAction, this.submitAction});

  State<_MyMonaStyleAttributesForm> createState() => _MyMonaStyleAttributesFormState(this.formAction);
}


class _MyMonaStyleAttributesFormState extends State<_MyMonaStyleAttributesForm> {
  final FormAction? formAction;
    ...
    return BlocBuilder<MonaStyleAttributesFormBloc, MonaStyleAttributesFormState>(builder: (context, state) {
      ...
      if (state is MonaStyleAttributesFormInitialized) {
        ...
        children.add(
                  ...
                  StyleRegistry.registry().styleWithApp(widget.app).adminFormStyle().radioListTile(widget.app, context, 0, _routeBuilderSelectedRadioTile, 'slideRightToLeft', 'slideRightToLeft', !Apis.apis().getCoreApi().memberIsOwner(context, widget.app.documentID) ? null : (dynamic val) => setSelectionRouteBuilder(val))
          );
        children.add(

                  StyleRegistry.registry().styleWithApp(widget.app).adminFormStyle().radioListTile(widget.app, context, 0, _routeBuilderSelectedRadioTile, 'slideBottomToTop', 'slideBottomToTop', !Apis.apis().getCoreApi().memberIsOwner(context, widget.app.documentID) ? null : (dynamic val) => setSelectionRouteBuilder(val))
          );
        children.add(

                  StyleRegistry.registry().styleWithApp(widget.app).adminFormStyle().radioListTile(widget.app, context, 0, _routeBuilderSelectedRadioTile, 'scaleRoute', 'scaleRoute', !Apis.apis().getCoreApi().memberIsOwner(context, widget.app.documentID) ? null : (dynamic val) => setSelectionRouteBuilder(val))
          );
        children.add(

                  StyleRegistry.registry().styleWithApp(widget.app).adminFormStyle().radioListTile(widget.app, context, 0, _routeBuilderSelectedRadioTile, 'rotationRoute', 'rotationRoute', !Apis.apis().getCoreApi().memberIsOwner(context, widget.app.documentID) ? null : (dynamic val) => setSelectionRouteBuilder(val))
          );
        children.add(

                  StyleRegistry.registry().styleWithApp(widget.app).adminFormStyle().radioListTile(widget.app, context, 0, _routeBuilderSelectedRadioTile, 'fadeRoute', 'fadeRoute', !Apis.apis().getCoreApi().memberIsOwner(context, widget.app.documentID) ? null : (dynamic val) => setSelectionRouteBuilder(val))
          );

        children.add(

                  StyleRegistry.registry().styleWithApp(widget.app).adminFormStyle().textFormField(widget.app, context, labelText: 'Page Animation (millisec)', icon: Icons.text_format, readOnly: _readOnly(context, state), textEditingController: _routeAnimationDurationController, keyboardType: TextInputType.number, validator: (_) => state is RouteAnimationDurationMonaStyleAttributesFormError ? state.message : null, hintText: null)
          );
   ...
   */
  bool _readOnly(BuildContext context, MonaStyleAttributesFormInitialized state) {
